...
Test cases:
    ...
    other.deps.should_not_affect()
    other.self.deps.should_not_affect_2()
    
    a()
    a().b().c()
    ''.join(self.deps.k())
    
    ('a' * self.deps.x()).split(self.deps.y()) 
    
    self.deps.up(
        self.deps.a1(self.deps.a2(2)),
        b=self.deps.b(),
    )
    
    x = {'x': self.deps.in_dict_comp(v) for v in self.deps.also_in_dict_comprehension}
    x = {self.deps.in_set_comp(v) for v in self.deps.also_in_set_comprehension}
    z = ['x' + self.deps.in_list_comp(v) for v in self.deps.also_in_list_comprehension]
    z = ('x' + self.deps.in_gen_comp(v) for v in self.deps.also_in_gen_comprehension)
    
    "asdasdasdsd self.deps.not_in_string() "

    # self.deps.not_in_comments()
        
    value = self.deps.j().append(1)
    if True or self.deps.something_in_branch_predicate():
        self.deps.something_in_branch()
...
#
#
#     # method_source = textwrap.dedent(inspect.getsource(x))
#     # ast_tree = ast.parse(method_source)
#     # print(ast.dump(ast_tree, indent=4))
#
#     print(parse_deps_used_new(x))